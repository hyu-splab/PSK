...
class RequestChannel(val queueSize: Int,
  ...
  //requestQueue는 requestQueue_1과 동일. 기본 큐로 
  private val requestQueue = new ArrayBlockingQueue[BaseRequest](queueSize) //기본 큐
  //requestQueue metrics에 관한 코드 변경 아직 안함. 이후에 처리 해주고 이 주석 지우기 
  //04/02code 
  //private val requestQueue_1 = new ArrayBlockingQueue[BaseRequest](queueSize) 
  private val requestQueue_2 = new ArrayBlockingQueue[BaseRequest](queueSize) //priority queue
  ...
  */
  ...
  //기존 sendRequest함수 사용되는 코드 존재. 다 고칠 수 없으니, 오버로딩하기 
  def sendRequest(request: RequestChannel.Request , queue_id: Int): Unit = {
    queue_id match {
      case 1 =>
        requestQueue.put(request)
        //println(s"11111111111 request queue size : ${requestQueue.size} ")
        //println(s"111111111111111111111111111111111")
      case 2 =>
        requestQueue_2.put(request)
        //println(s"2222222222222222222222222222222222")
        //println(s"2222222222222222222 request queue size : ${requestQueue_2.size} ")
      case _ => throw new IllegalArgumentException(s"queue_id does not exist")
    }
  }
  ...
  */
  ...
  def receiveRequest(timeout: Long): RequestChannel.BaseRequest = {
    ...
    if (callbackRequest != null)
      ...
      //여기 작성 마무리하기 
      //val request = Option(requestQueue_2.poll(timeout, TimeUnit.MILLISECONDS)).getOrElse(requestQueue.take())
      //requestQueue_2에 timeout 거니깐 너무 느려짐 
      val request = Option(requestQueue_2.poll()).getOrElse(requestQueue.poll(timeout, TimeUnit.MILLISECONDS))
      request match{
        case WakeupRequest => callbackQueue.poll()
        case _ => request
      }
  ...
  //04/02code
  //receiveRequest()는 쓰이는곳 없어보임
  //아래 6개 함수는 일단 냅두고, 나중에 쓰이는 것이 확인되면 처리해주고 이 주석 지우기 
