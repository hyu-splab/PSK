          ...
          case Some(channel) =>
            ...
            if (header.apiKey == ApiKeys.SASL_HANDSHAKE && channel.maybeBeginServerReauthentication(receive,
              ...
              if (channel.serverAuthenticationSessionExpired(nowNanos)) {
                ...
                //03/19code
                println( s"SocketServer.scala processCompletedReceives() connectionId :  ${connectionId}, Processor id : ${id}")
                //03/20code
                println(s"request header for client id : ${header.clientId}");
          ...
          case Some(channel) =>
            ...
            if (header.apiKey == ApiKeys.SASL_HANDSHAKE && channel.maybeBeginServerReauthentication(receive,
              ...
              if (channel.serverAuthenticationSessionExpired(nowNanos)) {
                ...
                //여기서 검사
                /* 
                  * getFlag함수 return 값 : 설정되어 있으면 해당 request queue의 name return
                  * 0 이면 초기화 상태 (아직 설정 안 됨)
                  * 1 이면 기본 큐 
                  * 2 이상이면 priority queue
                */
                val channelFlag = channel.getFlag() 
                if(channelFlag != 0){ 
                  requestChannel.sendRequest(req,channelFlag)
                }
                else{ //channel이 생성되고 처음 받는 request. channel set 해줘야함 
                  // client id 조사 
                  val client_id = header.clientId
                  if(client_id == "my_first_client"){
                    //setFlag 2로
                    channel.setFlag(2)
                    requestChannel.sendRequest(req,2)
                  }
                  else{
                    //setFlag 1로 
                    channel.setFlag(1)
                    requestChannel.sendRequest(req,1)
                  }
                }
                //requestChannel.sendRequest(req)
                selector.mute(connectionId)
